...
public class EventCopyHelper {
  ...
  /**
   * Copies a single event from one calendar to another, using the provided start
   * time in the target calendar.
   *
   ...
   * @param sourceOccurrenceStart The start time of a known occurrence in the
   *                              source calendar.
   * @param targetStart           The desired start time for the corresponding
   *                              occurrence in the target calendar.
   ...
   * @return A report summarizing all copied events and any conflicts.
  ...
  public static String copyEventsOnDate(ICalendarContext sourceCal, ICalendarContext targetCal,
    ...
    for (Event e : events) {
      for (Event occ : e.getOccurrences()) {
        if (!occ.getStartDateTime().toLocalDate().equals(sourceDate)) continue;
  ...
  /**
   * Copies all events from a source date range into a target calendar,
   * starting from the given target date.
   *
   * @param sourceCal        The source calendar context.
   * @param targetCal        The target calendar context.
   ...
   * @return A report of copied events or a message if none were found.
  ...
  /**
   * Copies a single event or a recurring event series from one calendar to another,
   ...
   * @return A message indicating the result of the copy operation.
